...
object KafkaGenderCount {
    implicit val formats = DefaultFormats  // 数据格式化时需要
    def main(args: Array[String]): Unit={
        if (args.length < 3) {
            System.err.println("Usage: KafkaGenderCount <brokers> <groupId> <topics>")
            System.exit(1)
        }
        StreamingExamples.setStreamingLogLevels()
        val Array(brokers, groupId, topics) = args
        val SparkConf = new SparkConf().setAppName("KafkaGenderCount")
        val ssc = new StreamingContext(SparkConf, Seconds(1))
        // 这里表示要把检查点文件写入分布式文件系统HDFS，所以要启动Hadoop
        ...
        val wordCounts = words.map(x => (x, 1L))
            ...
            if(rdd.count !=0 ){
                ...
                // 封装成Kafka消息，topic为"cat_result"
                val message = new ProducerRecord[String, String]("gender_result", null, str)
                // 给Kafka发送消息
