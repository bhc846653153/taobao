...
object KafkaActionCount {
    implicit val formats = DefaultFormats  // 数据格式化时需要
    def main(args: Array[String]): Unit={
        if (args.length < 3) {
            System.err.println("Usage: KafkaActionCount <brokers> <groupId> <topics>")
            System.exit(1)
        }
        StreamingExamples.setStreamingLogLevels()
        val Array(brokers, groupId, topics) = args
        val SparkConf = new SparkConf().setAppName("KafkaActionCount")
        ...
        val wordCounts = words.map(x => (x, 1L))
            ...
            if(rdd.count !=0 ){
                ...
                // 封装成Kafka消息，topic为"xx_result"
                val message = new ProducerRecord[String, String]("action_result", null, str)
                // 给Kafka发送消息
